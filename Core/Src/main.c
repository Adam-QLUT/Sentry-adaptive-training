...
// 使用取模运算实现1, 2, 3, 4的循环请求
  uint8_t request_type = (tick_ms % 4) + 1;
  IMU_RequestData(&hfdcan3, 0x01, request_type);

  // 当tick_ms到达一个较大的值时清零，防止溢出。
  // 这里用4000是为了方便对齐，1000也可以。
  if (tick_ms >= 4000)
  {
    tick_ms = 0;
  }
