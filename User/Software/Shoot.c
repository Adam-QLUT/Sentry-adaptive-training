...
void Shoot_control()
{
			 ...
			 pre_shoot_mode == CLOSE) // 摩擦轮启动过程
	{
		...
		if ((abs(SHOOTMotor_get_data(ShootMotor_L).speed_rpm) <= SHOOT_SPEED_BEGIN + 300) &&
			(abs(SHOOTMotor_get_data(ShootMotor_L).speed_rpm) >= SHOOT_SPEED_BEGIN - 300) &&
			(abs(SHOOTMotor_get_data(ShootMotor_R).speed_rpm) <= SHOOT_SPEED_BEGIN + 300) &&
			(abs(SHOOTMotor_get_data(ShootMotor_R).speed_rpm) >= SHOOT_SPEED_BEGIN - 300))
		{
